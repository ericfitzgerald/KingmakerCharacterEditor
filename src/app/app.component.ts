...
export class AppComponent {
  ...
  if(character.Descriptor.Blueprint in companions){
    ...
    let template = this.resolveReferences(this.fs.readFileSync('templates/' + companions[descriptor.Blueprint] + '_template.json', {encoding:'utf8'}));
    this.recursiveBumpStatIds(template);
    recurseiveReplace(template, template, descriptor); //set correct owner
    ...
    for(let feature of descriptor.Progression.Features.m_Facts){ //Link Linzi ring to feature, what happens if we don't?
  ...
  else if(character == this.party.m_EntityData[0]){
